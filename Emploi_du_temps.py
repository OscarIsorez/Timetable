...
def generate_html_data(date_to_treat, color_palette, file_name, to_page, from_page=None):
    ...
    html_table += "</table>"
    html_page = f'<!DOCTYPE html><html lang="fr"><head><meta charset="iso-8859-2"><title>Emploi du temps</title><link rel="stylesheet" href="./style-{file_name[-2] + file_name[-1]}.css"><link rel="icon" href="./favicon.ico" type="image/x-icon" sizes="32x32"></head><body><p id="date">Derni?re Maj : {acutal_date.strftime("%m%h%d/%m/%Y")}</p>   '
